...
vario <- function(h,par=c(0,log(100),0),t=1){ ##return the semi-variogram
    ...
    lambda <- exp(lambda1+lambda2*reg.t[t]) + 5
...
getYear <- function(d){
    ...
    return(ifelse(d > WS & d < YEND, y+1,y))
}
...
extract_func <- function(id,res,data,vari="2t"){
    ncvar_get(data,varid=vari,start=c( (id - 1) %% res + 1, (id - 1) %/% res + 1, 1),count=c(1, 1,-1))
}
